...
<div class="flex justify-center pt-6 pb-6">
  <div class="block p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
    ...
    <div>
      <textarea v-model="notes.content" id="message" rows="4" class="p-2 resize-none w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Your note content..."></textarea>  
    </div>
    <div class="pt-2 flex justify-between">
      <button @click="Post()" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 mr-2 mb-2 h-12 w-36 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800">New</button>
  ...
  methods:{
    Post(){
      ...
      if (this.notes.title === ""){
        ...
        axios.post('http://localhost:3000/notes', newNote).then(res => console.log(res.data)),
